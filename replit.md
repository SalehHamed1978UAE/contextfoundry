...
### Key Metrics
- **ClaudeCode Nexus Industries (active vault `176a4fb2-0bb4-4da3-9068-0e26268fca71`):** 77/100 questions passing (measured 2026-05-08, post tree-retrieval HOLDS_POSITION matcher fix + role-match confidence boost — see Tree Retrieval Investigation below). **v1 ceiling.**
...
**Tree matcher fix — HOLDS_POSITION + affiliation traversal (2026-05-08, third iteration):** Diagnosis from prior B1 merge run: BFS structurally reached the canonical anchor but `_matches_intent` rejected every PERSON candidate because (a) the hardcoded 7-entry `RELATIONSHIP_TO_ROLE` map didn't recognize `HOLDS_POSITION`, and (b) the ROLE intent's `relationship_types` list didn't include `WORKS_AT`/`AFFILIATED_WITH`/`REPORTS_TO`, so BFS couldn't reach PERSON entities from ORG anchors at all. Two surgical changes:
1. `intent_extractor.py` L72-74: added `WORKS_AT, AFFILIATED_WITH, REPORTS_TO` to ROLE intent's `relationship_types` so BFS can reach persons via affiliation edges.
2. `tree_retriever.py`: threaded `parent` through the BFS queue (L188-235) into `_matches_intent` as `neighbor` arg. Extended ROLE-query matching with three paths: (a) static map fallback for legacy edges (CEO_OF/CFO_OF/...); (b) when BFS arrived via HOLDS_POSITION/HAS_ROLE/HOLDS_ROLE, the neighbor IS the role entity — match its name against the requested role; (c) when a PERSON is reached via affiliation, query their outgoing HOLDS_POSITION edges via new `_get_person_role_names` helper (L444-469) and accept if any role-neighbor name contains the requested role. Allows STAGING+TRUSTED relationships.
**Result: 76/100, +2 vs baseline (Q75 "VP of Engineering for Nexus Digital Solutions" and Q76 "patent portfolio size" both recovered), 0 regressions.** Both recoveries are PERSON-via-affiliation cases — exactly the path (c) shape.

**Role-match confidence boost (2026-05-08, fourth iteration):** To bypass the hybrid gate's `'high'` confidence requirement when role matching is high-precision, tagged BFS results with `role_match=True` when validated via path (b) or (c) (`tree_retriever.py` L214-240), and modified `_compute_confidence` (L674-681) to short-circuit to `'high'` whenever any of the top-3 results has `role_match=True`. **Result: 77/100, +3 vs baseline (Q29 added to Q75 + Q76 recoveries), 0 regressions.** Q1/Q14/Q37/Q45/Q61/Q68/Q91 still fail — these are not being reached by the tree path at all (likely intent classifier not tagging them as ROLE, or `_is_graph_hopping_query` regex not matching) since the role-match boost would have fired if BFS had returned them.

**v1 ceiling: 77/100.** Per user directive on 2026-05-08 ("if we hit 80, commit and stop; if not, accept 76 as v1 ceiling"), iteration stopped here. Accepted ceiling raised to 77 by the role-match boost. Code retained behind `CF_TREE_BASED_RETRIEVAL=true` (start.sh L13, runner defaults). Future work to reach 80+ requires: (a) audit `IntentExtractor.extract()` and `_is_graph_hopping_query` for the 7 still-failing graph-hopping queries to confirm both correctly fire on them; (b) for Q14/Q45 (date queries — "When was X appointed"), tree retrieval doesn't help because dates live in document chunks; need a different fix path. Files: `src/context_foundry/retrieval/tree_retriever.py`, `src/context_foundry/retrieval/intent_extractor.py`, `src/context_foundry/agents/retrieval_router.py`.
