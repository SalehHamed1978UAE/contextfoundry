...
### Key Metrics
- **ClaudeCode Nexus Industries (active vault `176a4fb2-0bb4-4da3-9068-0e26268fca71`):** 76/100 questions passing (measured 2026-05-08, post tree-retrieval HOLDS_POSITION matcher fix — see Tree Retrieval Investigation below)
...
**Result: 76/100, +2 vs baseline (Q75 "VP of Engineering for Nexus Digital Solutions" and Q76 "patent portfolio size" both recovered), 0 regressions.** Both recoveries are PERSON-via-affiliation cases — exactly the path (c) shape. Did NOT recover Q1/Q14/Q37/Q45/Q61/Q68/Q91. **Why Q1 still fails:** the `Dr. Victoria Chen → HOLDS_POSITION → CEO` edges exist in STAGING (verified via SQL), and my matcher correctly handles STAGING — but Q1's answer cites "VP Trade Compliance" (a ROLE entity) which my matcher would reject. That answer is coming from the **legacy router** because the hybrid gate (`_is_graph_hopping_query` + `'high'` confidence threshold at retrieval_router.py L2079) is falling back to legacy when tree confidence < high. The tree-retrieval code is now correct; the next bottleneck is `_compute_confidence` (L570-590) which doesn't reach `'high'` for these BFS paths even when they return correct entities. Recommended next step: instrument tree confidence per-query, then either lower gate threshold to `medium` for graph-hopping queries OR boost confidence when ROLE-match path (b)/(c) succeeds (high signal that we found the right role). Code retained behind `CF_TREE_BASED_RETRIEVAL=true` (currently true in start.sh L13 and runner defaults).
