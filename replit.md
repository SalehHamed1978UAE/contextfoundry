...
- **Role→Attribute/Relationship Query Chaining**: Query pipeline rewrites role-based queries (e.g., "CEO's salary") into person-specific queries, handling single and multi-matches with context prioritization. **Policy Intent Detection** prevents chaining for rate/policy queries about job levels (e.g., "referral bonus for a Director") by recognizing indefinite "for a [role]" patterns as POLICY intent rather than PERSON intent.
