...
class LearningTicketAgent:
    ...
    def _increment_ticket_priority(self, ticket_id: str):
        """Increment hit count and boost priority using explicit formula."""
        
        # Formula: priority = base_severity * (1 + 0.2 * hit_count), capped at 1.0
        self.session.execute(
            text("""
                UPDATE learning_tickets
                SET hit_count = hit_count + 1,
                    priority = LEAST(1.0, (context->>'severity')::float * (1 + 0.2 * (hit_count + 1))),
                    updated_at = NOW()
                WHERE id = :ticket_id
            """),
            {'ticket_id': ticket_id}
...
When a ticket is resolved, the `resolution_notes` must include:
```json
{
  ...
  "source_chunks_used": ["chunk_id_1", "chunk_id_2"],
...
This enables:
- Audit trail: What was added, from where
- Rollback: Which facts came from which ticket
- Metrics: Learning effectiveness over time
...
class GardenerLearningProcessor:
    ...
    async def _process_ticket(self, ticket: dict) -> dict:
        ...
        try:
            if ticket['gap_type'] == 'missing_entity':
                ...
                result = {'action': 'ignored', 'improved': False, 'notes': f"Unknown gap type: {ticket['gap_type']}"}
            
            # Build structured resolution payload
            resolution_payload = {
            ...
            # Mark as resolved with structured payload
            ...
            self._update_ticket_status(ticket['id'], 'pending', {'error': str(e)})
            raise
    ...
    def _update_ticket_status(self, ticket_id: str, status: str, resolution: dict = None):
