...
For Q9 (CEO vs Chief Executive Officer), add to `evaluator.py`:

**In `FuzzyEvaluator.__init__`**:
```python
self.TITLE_EXPANSIONS = {
...
**Add method**:
```python
def _check_title_match(self, expected: str, actual: str) -> bool:
    exp_lower = expected.lower().strip()
    act_lower = actual.lower().strip()

    for abbr, full in self.TITLE_EXPANSIONS.items():
        if exp_lower == abbr and full in act_lower:
            return True
        if full in exp_lower and abbr == act_lower:
            return True
    return False
```

**Call it in `evaluate()`** before returning no_match.

This is domain-general knowledge (standard English abbreviations), not corpus-specific hardcoding.
...
| Title abbreviations | evaluator.py | Standard English mappings |

**Zero corpus-specific hardcoding. All intelligence comes from LLMs and existing components.**
