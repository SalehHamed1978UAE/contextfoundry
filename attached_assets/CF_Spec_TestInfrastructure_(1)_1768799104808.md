...
1. **USE THE API, NOT DIRECT DATABASE ACCESS** — All operations (create vault, upload docs, query, check status) must go through the CF API endpoints. This ensures:
   - User has proper permissions to access the vault in the UI
   - All business logic (RLS, validation, etc.) is applied
   - Vault appears in user's dashboard
   - No orphaned data or permission issues
   
   **NEVER** insert directly into PostgreSQL tables. Always use `/api/vaults`, `/api/vaults/{id}/documents`, `/api/vaults/{id}/query`, etc.

2. **RUN TESTS SEQUENTIALLY** — Never run multiple corpora in parallel. One at a time.

3. **WAIT FOR EXTRACTION PROPERLY** — Three phases:
