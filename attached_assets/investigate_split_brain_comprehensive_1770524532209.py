...
def main():
    ...
    for schema in ['platform', 'public']:
        ...
        if count > 0:
            avail_cols = schema_info[f"{schema}.documents"]
            name_col = 'name' if 'name' in avail_cols else ('title' if 'title' in avail_cols else None)
            select_cols = ['id']
            if name_col:
                select_cols.append(name_col)
            if 'status' in avail_cols:
                select_cols.append('status')
            if 'extraction_level' in avail_cols:
                select_cols.append('extraction_level')
            if 'single_extracted_at' in avail_cols:
                select_cols.append('single_extracted_at')
            if 'gpt_extracted_at' in avail_cols:
                select_cols.append('gpt_extracted_at')
            if 'multi_extracted_at' in avail_cols:
                select_cols.append('multi_extracted_at')

            cursor.execute(f"""
                SELECT {', '.join(select_cols)}
                FROM {schema}.documents
                WHERE {id_col} = %s
                ORDER BY {name_col or 'id'}
                LIMIT 5;
            """, (vault_id,))

            samples = cursor.fetchall()
            print(f"\nSample documents (showing up to 5):")
            header_cols = [c for c in select_cols if c != 'id']
            print("  ".join(f"{c:<30}" for c in header_cols))
            print("─" * 100)
            for doc in samples:
                vals = []
                for c in header_cols:
                    ...
                    if v is None:
                        v = '✗'
                    elif isinstance(v, bool) or (hasattr(v, '__class__') and 'datetime' in v.__class__.__name__):
                        v = '✓'
    ...
    for schema in ['platform', 'public']:
        if schema in entity_data and entity_data[schema]:
            ...
            for row in entity_data[schema]:
                stage_value = str(list(row.values())[0]).lower()
