...
class ContextFoundry:
    ...
    def __init__(
        ...
        # Financial query handler for pre-calculated metrics
        self.financial_handler = FinancialQueryHandler(self.session, tenant_id)
    ...
    def query(
        ...
        try:
            ...
            # Try financial query handler first for pre-calculated metrics
            financial_result = self.financial_handler.handle_query(query_text)
            if financial_result:
                query_logger.log_event("FINANCIAL_QUERY_HANDLED", {
                    "metric_type": financial_result.get('metric_type'),
                    "source": financial_result.get('source'),
                })
                response = {
                    "answer": financial_result['answer'],
                    "confidence": financial_result.get('confidence', 0.95),
                    "confidence_level": "high",
                    "source": "pre_calculated_financial_metric",
                    "evidence_chain": [{
                        "type": "CALCULATED_METRIC",
                        "metric_type": financial_result.get('metric_type'),
                        "time_period": financial_result.get('time_period'),
                        "raw_value": financial_result.get('raw_value'),
                        "unit": financial_result.get('unit'),
                        "formula": financial_result.get('formula'),
                    }],
                    "rules_applied": [],
                    "caveats": [],
                }
                if save_to_log:
                    self._save_query_log(query_id, query_text, response)
                return response
            ...
            # Apply tri-memory precedence: Symbolic > Semantic > Episodic
            # This checks symbolic rules and data gates before validation
            try:
                precedence_answer, precedence_source, precedence_confidence = apply_precedence(
                    ...
                    bundle=bundle,
                    context={
                        "target_entity": bundle.target_entity_found,
                        "target_entity_name": bundle.target_entity_name,
                        "entity_types": [e.get("entity_type", "") for e in bundle.semantic_entities if isinstance(e, dict)],
                    }
                )

                # Update response with precedence results
                response["answer"] = precedence_answer
                response["answer_source"] = precedence_source
                response["precedence_confidence"] = precedence_confidence

                # If answer was overridden or refused, log it
                if precedence_source in ("symbolic", "refused"):
                    query_logger.log_event("PRECEDENCE_APPLIED", {
                        "source": precedence_source,
                        "confidence": precedence_confidence,
                        "overridden": precedence_source == "symbolic",
                        "refused": precedence_source == "refused"
                    })

            except Exception as e:
                logger.warning(f"Precedence pipeline error (using original answer): {e}")
                response["answer_source"] = "semantic"
                response["precedence_error"] = str(e)
