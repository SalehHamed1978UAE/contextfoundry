...
Run with: python test_precedent_middleware.py

These tests verify the "look before you leap" behavior is working correctly.
"""
...
from precedent_middleware import PrecedentMiddleware, decide_with_context, Precedent
...
class MiddlewareTests:
    def __init__(self):
        ...
        self.decision_maker_id = "test-agent"
        self.results = {}
    ...
    def test_2_retrieve_precedents(self):
        ...
        precedents = middleware.retrieve_precedents(
            situation="Customer wants a discount due to churn risk",
            decision_type="discount_approval",
            limit=5
        )
        
        print_info(f"Found {len(precedents)} precedents")
        ...
        if precedents:
            ...
            self.results["retrieve"] = True  # Not a failure, just no data
    ...
    def test_4_decide_with_precedents_no_match(self):
        ...
        def decide_fn(precedents):
            # Always use default (for testing)
            return {
        ...
        if result.choice and result.deviated:
            print_pass(f"Correctly handled no-match scenario")
            print_info(f"Choice: {result.choice}")
            print_info(f"Deviation reason: {result.deviation_reason}")
    ...
    def test_5_decide_with_precedents_follow(self):
        ...
        def decide_fn(precedents):
            if precedents and precedents[0].score >= 0.5:  # Lower threshold for testing
                return {
        ...
        if result.precedent_followed:
            ...
            self.results["follow"] = True  # Not a failure
        else:
    ...
    def test_6_decide_with_precedents_deviate(self):
        ...
        def decide_fn(precedents):
            if precedents:
                # Explicitly deviate from top precedent
                return {
    ...
    def print_summary(self):
        ...
        if failed == 0:
            ...
            print(f"\n{BLUE}The 'look before you leap' pattern is working.{RESET}")
            print(f"{BLUE}Agents can now query precedents before making decisions.{RESET}")
        else:
            print(f"\n{RED}⚠️  Some tests failed. Review output above.{RESET}")
