...
This script:
1. Scans all existing documents in a vault/tenant
2. Re-runs the post-processor to find missed relationships
3. Standardizes all relationship types
4. Detects and reports extraction gaps
5. Optionally fixes gaps automatically
...
class ExtractionBackfiller:
    ...
    Re-runs post-processor and gap detection on all documents
    in a vault to catch missing relationships.
    """
    ...
    def run(self) -> BackfillResult:
        ...
        try:
            # Step 1: Standardize existing relationship types
    ...
    def _process_document(self, doc: Document) -> Dict:
        ...
        # Get existing entities and relationships for this document
    ...
    def _get_existing_entities(self, doc_id: uuid.UUID) -> List[Dict]:
        """Get existing entities for a document."""
        entities = self.session.query(Entity).filter(
            Entity.source_document_id == str(doc_id)
        ).all()
    ...
    def _get_existing_relationships(self, doc_id: uuid.UUID) -> List[Dict]:
        """Get existing relationships for a document."""
        relationships = self.session.query(Relationship).filter(
            Relationship.source_document_id == str(doc_id)
        ).all()

        # Need to get entity names
        result = []
        for rel in relationships:
    ...
    def _resolve_entity_id(self, name: str, tenant_id: uuid.UUID) -> Optional[uuid.UUID]:
    ...
    def _detect_gaps(self):
        """Run gap detection."""
        detector = ExtractionGapDetector(
    ...
    def _fix_gaps(self, gaps) -> int:
        ...
        # For now, just log gaps - fixing requires more complex logic
        fixed = 0
        for gap in gaps:
            logger.info(f"[Backfill] Gap: {gap.gap_type.value} - {gap.entity_name}: {gap.description}")
            # Future: implement automatic fixes for certain gap types
        return fixed


def main():
    parser = argparse.ArgumentParser(
        description="Backfill extraction for existing documents"
    )
    ...
    group.add_argument(
        "--tenant-id",
        type=str,
        help="UUID of tenant/vault to process"
    )
    group.add_argument(
        "--all-tenants",
        action="store_true",
        help="Process all tenants"
    )
    ...
    parser.add_argument(
        "--fix-gaps",
        action="store_true",
        help="Attempt to automatically fix detected gaps"
    )
